...
DOMAIN_SHAPE = {
    ...
    # Step 16.2 — one scored document per call (sync, no media artifacts).
    "viral": SHAPE_SYNC_DOCUMENT,
}
...
DEFAULT_CAPABILITIES = {
    ...
    "viral": {
        "test_connection": True,
        "single_scene": True,
        # Step 16.2 defaults. `offline` stays False because a scaffolded judge
        # is far more likely to call a model than not; the author flips it back
        # on if the implementation really never leaves the process.
        "script_scoring": True,
...
def _manifest_source(
    *,
    ...
    return f'''"""Provider manifest — Provider Contract v2.
...
def _settings_schema_source(*, label: str, kind: str, requires: list[str]) -> str:
    ...
    for key, value in props.items():
        ...
        props_lines.append("            },")
    props_body = "\n".join(props_lines)
    return f'''"""Provider settings schema.
...
def _provider_source_script(*, name_cap: str, label: str, provider_id: str) -> str:
    ...
    return f'''"""Script provider — Provider Contract v2.
...
def _provider_source_scene_blueprint(*, name_cap: str, label: str, provider_id: str) -> str:
    return f'''"""Scene-blueprint provider — Provider Contract v2 skeleton.
...
class {name_cap}Provider(SceneBlueprintProvider):
    """Scene-blueprint provider implementation."""
...
def _provider_source_tts(*, name_cap: str, label: str, provider_id: str) -> str:
    return f'''"""TTS provider — Provider Contract v2 skeleton.
...
def _provider_source_review(*, name_cap: str, label: str, provider_id: str) -> str:
    ...
    return f'''"""Review provider — Provider Contract v2 skeleton.
...
def _provider_source_viral(*, name_cap: str, label: str, provider_id: str) -> str:
    """Skeleton for the viral domain (step 16.2) — one typed score per call."""
    return f'''"""Viral provider — Provider Contract v2 skeleton.

Generated by `python -m scriptase.providers.scaffold`.
Implement `score()` to return a `ViralResultPayload`. The dimension ids, the
weights, and the band floors are the frozen contract: fill in the same
breakdown the deterministic scorer does, or the Script panel cannot render it.
"""
...
from scriptase.modules.viral.providers.contract import ViralRequest, ViralResultPayload


class {name_cap}Provider(ViralProvider):
    """Virality provider implementation."""
    ...
    def score(
        ...
        *,
        ...
        raise NotImplementedError("{name_cap}Provider.score is not implemented")
...
def _provider_source_async(
    *,
    ...
    return f'''"""{domain.title()} provider — Provider Contract v2 skeleton.
...
class {name_cap}Provider({base_class}):
    """{domain.title()} provider implementation."""
...
def _runtime_source(*, provider_id: str, label: str) -> str:
    return f'''"""Optional WebSocket runtime for an extension provider.
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.no_docs:
        ...
        if _run(["-m", "scriptase.engine.docs"], cwd=root) != 0:
            ...
            return 1
        print("Guide: docs/provider-author-guide.md")
