"""Forward-only schema migrations for Scene identity documents.

Mirrors ``scriptase.artifacts.migrations``: ``MIGRATIONS`` maps a **target**
...
# Freshly written Scene documents carry this schema_version.
...
def apply_migrations(data: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    if not isinstance(data, dict):
        raise TypeError("scene document must be an object")
